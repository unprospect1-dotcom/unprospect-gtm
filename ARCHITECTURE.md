...
- Del repo de Owoslawski (github.com/growthenginenowoslawski/coldoutboundskills) los equivalentes conceptuales son: `lead-magnet-brainstorm` → `gtm-offer-ideation`, `campaign-strategy` → `gtm-campaign-ideation`, `positive-reply-scoring` → `gtm-reply-analysis`, `experiment-design` → `gtm-experiments`, `icp-onboarding` → `gtm-onboard`. La diferencia clave de nuestra versión: **memoria persistente por workspace + auto-aprendizaje por skill**, que ese repo no tiene. Sus skills de infraestructura (deliverability, domains, Smartlead) se pueden adoptar tal cual más adelante si hacen falta.

...
2. **`gtm-list-build`** — Lista lista para campaña. Prospeo (patrones del skill de referencia) / AI ARK / DiscoLike opcional / lista que el usuario trae (ej. lookalikes). Pipeline: buscar → dedupe contra `outreach_log` (via gtm-check-contact) → scorecard de calidad con letter grade → prompt de calificación afinado por lotes de 10 (patrón icp-prompt-builder). Artefacto: CSV graduado + reporte.
